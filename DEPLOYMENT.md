...
板子（Orange Pi Zero 3）
   │  停服务 → 压缩流传二进制 → 重启
   ▼
/opt/monitor/  （systemd: monitor，监听 :8080）
```
...
|---|---|
| 部署目录 | `/opt/monitor/` |
| 二进制 | `/opt/monitor/monitor_server`（含内嵌前端） |
| systemd 服务 | `monitor.service` |
| 监听端口 | `8080` |
| 公网入口 | `monitor.<your-domain>`（Cloudflare 隧道 → http://localhost:8080） |
...
| `/opt/monitor/` | 生产部署目录 | 由 CI 更新 |
| `monitor.service` | systemd 单元，指向 `/opt/monitor/monitor_server` | 常驻 |
...
| Public Hostname | `monitor.<your-domain>` → `http://localhost:8080` | 公网访问 Web |
...
3. 加 Public Hostname：
   - `monitor.<your-domain>` → `http://localhost:8080`
...
板子 `/opt/monitor/monitor_server` 是否更新、服务是否重启。
...
- 或手动替换 `/opt/monitor/monitor_server` 为上一版二进制并重启
